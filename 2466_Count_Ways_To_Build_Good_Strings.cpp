...
public:
    int M = 1e9+7; 
    int solve(int low ,int high,int zero,int one, int count, vector<int> &dp){
        if(count>high){
            return 0;
        }
        if(dp[count] != -1){
        ...
        int takeZero = solve(low, high, zero, one, count + zero, dp);
        int takeOne = solve(low, high, zero, one, count + one, dp);
        return dp[count] = (continued + takeZero + takeOne)%M;
    }
